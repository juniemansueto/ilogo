...
namespace ILOGO\Logoinc\Traits;
...
trait Translatable
{
    ...
    {
        return $this->hasMany(Logoinc::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('logoinc.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('logoinc.multilingual.locales', [config('logoinc.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('logoinc.multilingual.default'), false];
        }
        ...
        $default = config('logoinc.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('logoinc.multilingual.default', 'en');
        $locales = config('logoinc.multilingual.locales', [$default]);
    ...
    public function prepareTranslations($request)
    {
        ...
        foreach ($fields as $field) {
            ...
            $request->merge([$field => $trans[config('logoinc.multilingual.default', 'en')]]);
    ...
    public function prepareTranslationsFromArray($field, &$requestData)
    {
        ...
        // Set the default local value
        $requestData['display_name'] = $trans[config('logoinc.multilingual.default', 'en')];
