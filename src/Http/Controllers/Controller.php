...
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Checkbox;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Coordinates;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\File;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Image as ContentImage;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\MultipleCheckbox;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\MultipleImage;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Password;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Relationship;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\SelectMultiple;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Text;
use ILOGO\Logoinc\Http\Controllers\ContentTypes\Timestamp;
use ILOGO\Logoinc\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            $data->save();
            if ($old_path != $new_path && !Storage::disk(config('logoinc.storage.disk'))->exists($new_path)) {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('logoinc.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('logoinc.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if (Storage::disk(config('logoinc.storage.disk'))->exists($path)) {
            Storage::disk(config('logoinc.storage.disk'))->delete($path);
