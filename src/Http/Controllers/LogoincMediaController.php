...
class LogoincMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('logoinc.storage.disk');
    }
    ...
    public function index()
    {
        ...
        return Logoinc::view('logoinc::media.index');
    }
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('logoinc.hidden_files')) {
                    continue;
                }
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('logoinc::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('logoinc::media.error_creating_dir');
        }
        ...
        foreach ($request->get('files') as $file) {
            ...
            if ($file['type'] == 'folder') {
                if (!Storage::disk($this->filesystem)->deleteDirectory($file_path)) {
                    $error = __('logoinc::media.error_deleting_folder');
                    $success = false;
                }
            } elseif (!Storage::disk($this->filesystem)->delete($file_path)) {
                $error = __('logoinc::media.error_deleting_file');
                $success = false;
            }
        }
    ...
    public function rename(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                ...
                $error = __('logoinc::media.error_moving');
            }
        } else {
            $error = __('logoinc::media.error_may_exist');
        }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('logoinc.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('logoinc::generic.mimetype_not_allowed'));
            }
            ...
            $success = true;
            $message = __('logoinc::media.success_uploaded_file');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('logoinc::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
