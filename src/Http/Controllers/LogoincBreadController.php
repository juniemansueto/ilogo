...
class LogoincBreadController extends Controller
{
    public function index()
    {
        ...
        return Logoinc::view('logoinc::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = Logoinc::model('DataType')->whereName($table)->first();
        ...
        return Logoinc::view('logoinc::tools.bread.edit-add', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = Logoinc::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('logoinc::bread.success_created_bread'))
                : $this->alertError(__('logoinc::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('logoinc.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('logoinc.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = Logoinc::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return Logoinc::view('logoinc::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \ILOGO\Logoinc\Models\DataType $dataType */
        try {
            $dataType = Logoinc::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('logoinc::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('logoinc::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('logoinc::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = Logoinc::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('logoinc::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('logoinc::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            Logoinc::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = Logoinc::modelClass('DataRow');
            ...
            $newRow->order = intval(Logoinc::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Logoinc::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Logoinc::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        Logoinc::model('DataRow')->destroy($id);
