...
use ILOGO\Logoinc\Events\BreadDataRestored;
use ILOGO\Logoinc\Events\BreadDataUpdated;
use ILOGO\Logoinc\Events\BreadImagesDeleted;
use ILOGO\Logoinc\Facades\Logoinc;
use ILOGO\Logoinc\Http\Controllers\Traits\BreadRelationshipParser;

class LogoincBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($dataType->server_side) {
            ...
            $dataRow = Logoinc::model('DataRow')->whereDataTypeId($dataType->id)->get();
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Logoinc::actions() as $action) {
        ...
        $view = 'logoinc::bread.browse';

        if (view()->exists("logoinc::$slug.browse")) {
            $view = "logoinc::$slug.browse";
        }

        return Logoinc::view($view, compact(
            'actions',
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
    ...
    public function show(Request $request, $id)
    {
        ...
        $view = 'logoinc::bread.read';

        if (view()->exists("logoinc::$slug.read")) {
            $view = "logoinc::$slug.read";
        }

        return Logoinc::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'logoinc::bread.edit-add';

        if (view()->exists("logoinc::$slug.edit-add")) {
            $view = "logoinc::$slug.edit-add";
        }

        return Logoinc::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', $model)) {
            $redirect = redirect()->route("logoinc.{$dataType->slug}.index");
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('logoinc::generic.successfully_deleted')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('logoinc::generic.error_deleting')." {$displayName}",
                'alert-type' => 'error',
            ];
        ...
        return redirect()->route("logoinc.{$dataType->slug}.index")->with($data);
    }
    ...
    public function restore(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('logoinc::generic.successfully_restored')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('logoinc::generic.error_restoring')." {$displayName}",
                'alert-type' => 'error',
            ];
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('logoinc::generic.field_does_not_exist'), 400);
            }
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('logoinc::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('logoinc::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
                'data' => [
                    'status'  => 200,
                    'message' => __('logoinc::media.file_removed'),
                ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('logoinc::generic.internal_error');
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('logoinc.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (!isset($dataType->order_column) || !isset($dataType->order_display_column)) {
            ...
            ->with([
                'message'    => __('logoinc::bread.ordering_not_set'),
                'alert-type' => 'error',
            ]);
        }
        ...
        $dataRow = Logoinc::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'logoinc::bread.order';

        if (view()->exists("logoinc::$slug.order")) {
            $view = "logoinc::$slug.order";
        }
