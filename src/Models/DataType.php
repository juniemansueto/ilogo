...
class DataType extends Model
{
    ...
    {
        return $this->hasMany(Logoinc::modelClass('DataRow'))->orderBy('order');
    }
    ...
    {
        return $this->hasMany(Logoinc::modelClass('DataRow'))->orderBy('order', 'DESC')->first();
    }
        ...
        try {
            ...
            if ($this->fill($requestData)->save()) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (!$dataRow->save()) {
                        throw new \Exception(__('logoinc::database.field_safe_failed', ['field' => $field]));
                    }
                ...
                if ($this->generate_permissions) {
                    Logoinc::model('Permission')->generateFor($this->name);
                }
