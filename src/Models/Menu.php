...
use ILOGO\Logoinc\Events\MenuDisplay;
...
class Menu extends Model
{
    ...
    {
        return $this->hasMany(Logoinc::modelClass('MenuItem'));
    }
    ...
    public static function display($menuName, $type = null, array $options = [])
    {
        ...
        $menu = \Cache::remember('logoinc_menu_'.$menuName, \Carbon\Carbon::now()->addDays(30), function () use ($menuName) {
        ...
        if ($type == 'admin') {
            $type = 'logoinc::menu.'.$type;
        } else {
            if (is_null($type)) {
                $type = 'logoinc::menu.default';
            } elseif ($type == 'bootstrap' && !view()->exists($type)) {
                $type = 'logoinc::menu.bootstrap';
            }
        }
    ...
    {
        \Cache::forget('logoinc_menu_'.$this->name);
    }
    ...
    private static function processItems($items)
    {
        ...
        $items = $items->transform(function ($item) {
            ...
            if (($item->href == url('') || $item->href == route('logoinc.dashboard')) && $item->children->count() > 0) {
                // Exclude sub-menus
                $item->active = false;
            } elseif ($item->href == route('logoinc.dashboard') && url()->current() != route('logoinc.dashboard')) {
                // Exclude dashboard
                $item->active = false;
            }
