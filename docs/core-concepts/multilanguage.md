...
Logoinc supports multiple languages for your models.To get started, you need to configure some things first.

## Setup

First you need to define some `locales` in your `config/logoinc.php` file and `enable` multilanguage:
...
```php
// with string
if (Logoinc::translatable(Post::class)) {
    // it's translatable
}

// with object of Model or Collection
if (Logoinc::translatable($post)) {
    // it's translatable
}
```
