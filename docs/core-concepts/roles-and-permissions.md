...
Logoinc comes with Roles and Permissions out of the box. Each _User_ has a _Role_ which has a set of _Permissions_.
...
New in version 1.0, we've changed Logoincs authorization system to be [more in line with Laravel](https://laravel.com/docs/authorization#authorizing-actions-using-policies)! This means that you can check for permissions in the following ways:
...
* `browse_admin`: Whether or not the user may browse the Logoinc admin panel.
* `browse_database`: Whether or not the user may browse the Logoinc database menu section.
* `browse_bread`: Whether or not the user may browse the Logoinc BREAD menu section.
* `browse_media`: Whether or not the user may browse the Logoinc media section.
* `browse_menu`: Whether or not the user may browse the Logoinc menu section.
* `browse_settings`: Whether or not the user may browse the Logoinc settings section.
...
```php
  ...
  {
      return Auth::guard(app('LogoincGuard'));
  }
```
