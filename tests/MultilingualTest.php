...
use ILOGO\Logoinc\Translator\Collection;
...
class MultilingualTest extends TestCase
{
    public function setUp(): void
    {
        ...
        config()->set('logoinc.multilingual.locales', ['en', 'da']);

        // Turn on multilingual
        config()->set('logoinc.multilingual.enabled', true);
    }

    public function testCheckingModelIsTranslatable()
    {
        $this->assertTrue(Logoinc::translatable(TranslatableModel::class));
        $this->assertTrue(Logoinc::translatable(ActuallyTranslatableModel::class));
    }

    public function testCheckingModelIsNotTranslatable()
    {
        $this->assertFalse(Logoinc::translatable(NotTranslatableModel::class));
        $this->assertFalse(Logoinc::translatable(StillNotTranslatableModel::class));
    }
