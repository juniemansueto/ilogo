...
class MenuTest extends TestCase
{
    ...
    public function testCanRenameMenu()
    {
        ...
        $this->visitRoute('logoinc.menus.edit', $menu->id)
             ->seeInField('name', $menu->name)
             ->type('new_admin', 'name')
             ->seeInElement('button', __('logoinc::generic.save'))
             ->press(__('logoinc::generic.save'))
             ->seePageIs(route('logoinc.menus.index'))
             ->seeInDatabase('menus', [
                 'id'   => $menu->id,
                 'name' => 'new_admin',
             ]);
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->delete(route('logoinc.menus.item.destroy', [
            'menu' => $menu->id,
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->post(route('logoinc.menus.item.add', [
            'menu'    => $menu->id,
            'menu_id' => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    public function testCanUpdateMenuItem()
    {
        ...
        $this->put(route('logoinc.menus.item.update', [
            'id'      => $item->id,
            'menu'    => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $response = $this->call('GET', route('logoinc.menus.builder', ['menu' => $menu->id]));
