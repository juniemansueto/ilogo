...
@if($db->action == 'update')
    @section('page_title', __('logoinc::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('logoinc::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        ...
        @if($db->action == 'update')
            {{ __('logoinc::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('logoinc::database.create_new_table') }}
        @endif
    </h1>
@stop
...
<ol class="breadcrumb hidden-xs">
    ...
    <li>
        <a href="{{ route('logoinc.database.index') }}">
            {{ __('logoinc::generic.database') }}
        </a>
    </li>

    @if($db->action == 'update')
    <li class="active">{{ __('logoinc::generic.edit') }}</li>
    <li class="active">{{ $db->table->name }}</li>
    @else
    <li class="active">{{ __('logoinc::generic.add') }}</li>
    @endif
</ol>
@endsection
...
@section('javascript')
    @include('logoinc::tools.database.vue-components.database-table-editor')
