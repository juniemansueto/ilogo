...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('logoinc::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('logoinc::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('logoinc::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('logoinc::generic.yes_please') }}" data-off="{{ __('logoinc::generic.no_thanks') }}">
            </div>
            {{--
                Hide migration button until feature is available.
                 <div class="col-md-3 col-sm-4 col-xs-6">
                    <label for="create_migration">{{ __('logoinc::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('logoinc::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('logoinc::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('logoinc::generic.name') }}</th>
                    <th>{{ __('logoinc::generic.type') }}</th>
                    <th>{{ __('logoinc::generic.length') }}</th>
                    <th>{{ __('logoinc::generic.not_null') }}</th>
                    <th>{{ __('logoinc::generic.unsigned') }}</th>
                    <th>{{ __('logoinc::generic.auto_increment') }}</th>
                    <th>{{ __('logoinc::generic.index') }}</th>
                    <th>{{ __('logoinc::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('logoinc::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('logoinc::database.update_table') }}@else{{ __('logoinc::database.create_new_table') }}@endif"
...
@include('logoinc::tools.database.vue-components.database-column')
@include('logoinc::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('logoinc::database.column') }} " + column.name + " {{ __('logoinc::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('logoinc::database.column') }} " + newName + " {{ __('logoinc::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('logoinc::database.table_has_index') }}");
                    }
