...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('logoinc::database.index') }}</option>
            <option value="UNIQUE">{{ __('logoinc::database.unique') }}</option>
            <option value="PRIMARY">{{ __('logoinc::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('logoinc::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="logoinc-trash"></i></div>
    </td>
...
@include('logoinc::tools.database.vue-components.database-types')
@include('logoinc::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('logoinc::database.name_warning') }}");
                }
