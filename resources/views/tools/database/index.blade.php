...
@section('page_title', __('logoinc::generic.viewing').' '.__('logoinc::generic.database'))

@section('page_header')
    <h1 class="page-title">
        <i class="logoinc-data"></i> {{ __('logoinc::generic.database') }}
        <a href="{{ route('logoinc.database.create') }}" class="btn btn-success"><i class="logoinc-plus"></i>
            {{ __('logoinc::database.create_new_table') }}</a>
    </h1>
@stop
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:right" colspan="2">{{ __('logoinc::database.table_actions') }}</th>
                        </tr>
                    </thead>
                ...
                @foreach($tables as $table)
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            ...
                            <a href="{{ route('logoinc.database.edit', $table->prefix.$table->name) }}"
                            ...
                            <a href="{{ route('logoinc.database.show', $table->prefix.$table->name) }}"
                               ...
                               <i class="logoinc-eye"></i> {{ __('logoinc::generic.view') }}
                            </a>
                        </td>
                    </tr>
                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="logoinc-trash"></i>  {!! __('logoinc::bread.delete_bread_quest', ['table' => '<span id="delete_bread_name"></span>']) !!}</h4>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('logoinc::generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="logoinc-trash"></i> {!! __('logoinc::database.delete_table_question', ['table' => '<span id="delete_table_name"></span>']) !!}</h4>
                </div>
                <div class="modal-footer">
                    <form action="#" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger pull-right" value="{{ __('logoinc::database.delete_table_confirm') }}">
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal">{{ __('logoinc::generic.cancel') }}
                        </button>
                    </form>
        ...
        $(function () {
            ...
            $('td.actions').on('click', '.delete_table', function (e) {
                ...
                if ($(this).hasClass('remove-bread-warning')) {
                    toastr.warning('{{ __('logoinc::database.delete_bread_before_table') }}');
                } else {
                    $('#delete_table_name').text(table);

                    $('#delete_table_form')[0].action = '{{ route('logoinc.database.destroy', ['database' => '__database']) }}'.replace('__database', table)
            ...
            $('table .bread_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_bread_form')[0].action = '{{ route('logoinc.bread.delete', '__id') }}'.replace('__id', id);
