...
<div class="row row-dd row-dd-relationship">
    <div class="col-xs-2">
        <h4><i class="logoinc-heart"></i><strong>{{ $relationship->getTranslatedAttribute('display_name') }}</strong></h4>
        <div class="handler logoinc-handle"></div>
        <strong>{{ __('logoinc::database.type') }}:</strong> <span>{{ __('logoinc::database.relationship.relationship') }}</span>
        <div class="handler logoinc-handle"></div>
    ...
    <div class="col-xs-2">
        ...
        <label for="field_browse_{{ $relationship['field'] }}"> {{ __('logoinc::database.relationship.browse') }}</label><br>
        ...
        <label for="field_read_{{ $relationship['field'] }}"> {{ __('logoinc::database.relationship.read') }}</label><br>
        ...
        <label for="field_edit_{{ $relationship['field'] }}"> {{ __('logoinc::database.relationship.edit') }}</label><br>
        ...
        <label for="field_add_{{ $relationship['field'] }}"> {{ __('logoinc::database.relationship.add') }}</label><br>
        ...
        <label for="field_delete_{{ $relationship['field'] }}"> {{ __('logoinc::database.relationship.delete') }}</label><br>
    </div>
    <div class="col-xs-2">
        <p>{{ __('logoinc::database.relationship.relationship') }}</p>
    </div>
    <div class="col-xs-2">
    ...
    <div class="col-xs-4">
        <div class="logoinc-relationship-details-btn">
            <i class="logoinc-angle-down"></i><i class="logoinc-angle-up"></i>
            <span class="open_text">{{ __('logoinc::database.relationship.open') }}</span>
            <span class="close_text">{{ __('logoinc::database.relationship.close') }}</span>
            {{ __('logoinc::database.relationship.relationship_details') }}
        </div>
    </div>
    <div class="col-md-12 logoinc-relationship-details">
        <a href="{{ route('logoinc.bread.delete_relationship', $relationship['id']) }}" class="delete_relationship"><i class="logoinc-trash"></i> {{ __('logoinc::database.relationship.delete') }}</a>
        <div class="relationship_details_content">
            ...
            <span><input type="text" class="form-control" name="relationship_model_{{ $relationship['field'] }}" placeholder="{{ __('logoinc::database.relationship.namespace') }}" value="{{ $relationshipDetails->model ?? '' }}"></span>
        </div>
        ...
        <div class="relationship_details_content margin_top">
            <label>{{ __('logoinc::database.relationship.display_the') }} <span class="label_table_name"></span></label>
            ...
            <label class="relationship_key" style="@if($relationshipDetails->type == 'belongsTo' || $relationshipDetails->type == 'belongsToMany') display:block @endif">{{ __('logoinc::database.relationship.store_the') }} <span class="label_table_name"></span></label>
            ...
            @isset($relationshipDetails->taggable)
                <label class="relationship_taggable" style="@if($relationshipDetails->type == 'belongsToMany') display:block @endif">
                    {{__('logoinc::database.relationship.allow_tagging')}}
                </label>
                <span class="relationship_taggable" style="@if($relationshipDetails->type == 'belongsToMany') display:block @endif">
                    <input type="checkbox" name="relationship_taggable_{{ $relationship['field'] }}" class="toggleswitch" data-on="{{ __('logoinc::generic.yes') }}" data-off="{{ __('logoinc::generic.no') }}" {{$relationshipDetails->taggable == 'on' ? 'checked' : ''}}>
                </span>
            @endisset
        </div>
        <div class="relationship_details_content margin_top">
            <div class="col-xs-12" style="margin: 0px !important; padding: 0px !important;">
                ...
                <label>{{ __('logoinc::database.relationship.relationship_details') }}</label>
