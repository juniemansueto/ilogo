...
@if (isset($dataType->id))
    @section('page_title', __('logoinc::bread.edit_bread_for_table', ['table' => $dataType->name]))
    @php
    ...
    @endphp
@else
    @section('page_title', __('logoinc::bread.create_bread_for_table', ['table' => $table]))
@endif

@section('page_header')
    <div class="page-title">
        <i class="logoinc-data"></i>
        @if (isset($dataType->id))
            {{ __('logoinc::bread.edit_bread_for_table', ['table' => $dataType->name]) }}
        @else
            {{ __('logoinc::bread.create_bread_for_table', ['table' => $table]) }}
        @endif
    </div>
    @php
    ...
    @endphp
    @include('logoinc::multilingual.language-selector')
@stop

@section('breadcrumbs')
<ol class="breadcrumb hidden-xs">
    <li class="active">
        <a href="{{ route('logoinc.dashboard')}}"><i class="logoinc-boat"></i> {{ __('logoinc::generic.dashboard') }}</a>
    </li>
    <li class="active">
        <a href="{{ route('logoinc.bread.index') }}">
            {{ __('logoinc::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('logoinc.bread.edit', $table) }}">
            {{ $display_name }}
        </a>
        @else
        <a href="{{ route('logoinc.bread.create', $table) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    <li>
        {{ isset($dataType->id) ? __('logoinc::generic.edit') : __('logoinc::generic.add') }}
    </li>
</ol>
@endsection

@section('content')
    <div class="page-content container-fluid" id="logoincBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('logoinc.bread.update', $dataType->id) }}@else{{ route('logoinc.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="logoinc-bread"></i> {{ ucfirst($table) }} {{ __('logoinc::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action logoinc-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('logoinc::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('logoinc::bread.display_name_singular') }}</label>
                                    @if($isModelTranslatable)
                                        @include('logoinc::multilingual.input-hidden', [
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('logoinc::bread.display_name_singular') }}"
                                           value="{{ $display_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('logoinc::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('logoinc::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('logoinc::bread.display_name_plural') }}"
                                           value="{{ $display_name_plural }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('logoinc::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('logoinc::bread.url_slug_ph') }}"
                                           value="{{ $dataType->slug ?? $slug }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('logoinc::bread.icon_hint') }} <a
                                                href="{{ route('logoinc.compass.index', [], false) }}#fonts"
                                                target="_blank">{{ __('logoinc::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('logoinc::bread.icon_class') }}"
                                           value="{{ $dataType->icon ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('logoinc::bread.model_name') }}</label>
                                    <span class="logoinc-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('logoinc::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('logoinc::bread.model_class') }}"
                                           value="{{ $dataType->model_name ?? $model_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('logoinc::bread.controller_name') }}</label>
                                    <span class="logoinc-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('logoinc::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('logoinc::bread.controller_name') }}"
                                           value="{{ $dataType->controller ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('logoinc::bread.policy_name') }}</label>
                                    <span class="logoinc-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('logoinc::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('logoinc::bread.policy_class') }}"
                                           value="{{ $dataType->policy_name ?? '' }}">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('logoinc::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox"
                                           ...
                                           class="toggleswitch"
                                           data-on="{{ __('logoinc::generic.yes') }}"
                                           data-off="{{ __('logoinc::generic.no') }}"
                                           @if($checked) checked @endif >
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('logoinc::bread.server_pagination') }}</label><br>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-3 form-group">
                                    <label for="order_column">{{ __('logoinc::bread.order_column') }}</label>
                                    <span class="logoinc-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('logoinc::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                        <option value="">-- {{ __('logoinc::generic.none') }} --</option>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_display_column">{{ __('logoinc::bread.order_ident_column') }}</label>
                                    <span class="logoinc-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('logoinc::bread.order_ident_column_ph') }}"></span>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_direction">{{ __('logoinc::bread.order_direction') }}</label>
                                    <select name="order_direction" class="select2 form-control">
                                        <option value="asc" @if(isset($dataType) && $dataType->order_direction == 'asc') selected @endif>
                                            {{ __('logoinc::generic.ascending') }}
                                        </option>
                                        <option value="desc" @if(isset($dataType) && $dataType->order_direction == 'desc') selected @endif>
                                            {{ __('logoinc::generic.descending') }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="default_search_key">{{ __('logoinc::bread.default_search_key') }}</label>
                                    <span class="logoinc-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('logoinc::bread.default_search_key_ph') }}"></span>
                            ...
                            <div class="row clearfix">
                                @if (isset($scopes) && isset($dataType))
                                    <div class="col-md-3 form-group">
                                        <label for="scope">{{ __('logoinc::bread.scope') }}</label>
                                ...
                                <div class="col-md-9 form-group">
                                    <label for="description">{{ __('logoinc::bread.description') }}</label>
                                    <textarea class="form-control"
                                              name="description"
                                              placeholder="{{ __('logoinc::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="logoinc-window-list"></i> {{ __('logoinc::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action logoinc-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('logoinc::database.field') }}</div>
                                <div class="col-xs-2">{{ __('logoinc::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('logoinc::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('logoinc::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('logoinc::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = Logoinc::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('field', '=', $data['field'])->first(); ?>
                                @endif

                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        <h4><strong>{{ $data['field'] }}</strong></h4>
                                        <strong>{{ __('logoinc::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('logoinc::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('logoinc::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('logoinc::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}" checked="checked">
                                        @else
                                            <span>{{ __('logoinc::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler logoinc-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                   checked="checked"
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('logoinc::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('logoinc::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('logoinc::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('logoinc::generic.add') }}</label><br/>
                                        <input type="checkbox"
                                    ...
                                    <div class="col-xs-2">
                                        @if($isModelTranslatable)
                                            @include('logoinc::multilingual.input-hidden', [
                                                ...
                                                '_field_trans' => $dataRow ? get_field_translations($dataRow, 'display_name') : json_encode([config('logoinc.multilingual.default') => ucwords(str_replace('_', ' ', $data['field']))]),
                                            ])
                                        @endif
                                        <input type="text" class="form-control"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('logoinc::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="logoinc-heart"></i> <span>
                             {{ __('logoinc::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('logoinc::generic.submit') }}</button>
...
@include('logoinc::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('logoinc::json.invalid_message') }}', '{{ __('logoinc::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
       ...
       $(function () {
            ...
            $('.logoinc-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.logoinc-relationship-details').slideDown();
                } else {
                    $(this).parent().parent().find('.logoinc-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            if(tbl.length != 0){
                $.get('{{ route('logoinc.database.index') }}/' + tbl, function(data){
                    $(dropdown).empty();
