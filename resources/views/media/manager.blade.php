...
<div>
                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon('{{ Storage::disk(config('logoinc.storage.disk'))->url('/') }}'+file)"></div>
                        </template>
                        <template v-else-if="fileIs(file, 'video')">
                            <i class="icon logoinc-video"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'audio')">
                            <i class="icon logoinc-music"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'zip')">
                            <i class="icon logoinc-archive"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'folder')">
                            <i class="icon logoinc-folder"></i>
                        </template>
                        <template v-else>
                            <i class="icon logoinc-file-text"></i>
                        </template>
                    </div>
                    <div class="details">
                        <div class="folder">
                    ...
                    <i class="logoinc-x dd-nodrag" v-on:click="removeFileFromInput(file)"></i>
                </div>
            </li>
        </ol>
    </div>
    <div v-if="hidden_element">
        <div class="btn btn-sm btn-default" v-on:click="expanded = !expanded;" style="width:100%">
            <div v-if="!expanded"><i class="logoinc-double-down"></i> {{ __('logoinc::generic.open') }}</div>
            <div v-if="expanded"><i class="logoinc-double-up"></i> {{ __('logoinc::generic.close') }}</div>
        </div>
    </div>
    <div id="toolbar" v-if="showToolbar" :style="expanded ? 'display:block' : 'display:none'">
        <div class="btn-group offset-right">
            <button type="button" class="btn btn-primary" id="upload" v-if="allowUpload">
                <i class="logoinc-upload"></i>
                {{ __('logoinc::generic.upload') }}
            </button>
            <button type="button" class="btn btn-primary" v-if="allowCreateFolder" data-toggle="modal" :data-target="'#create_dir_modal_'+this._uid">
                <i class="logoinc-folder"></i>
                {{ __('logoinc::generic.add_folder') }}
            </button>
        </div>
        <button type="button" class="btn btn-default" v-on:click="getFiles()">
            <i class="logoinc-refresh"></i>
        </button>
        <div class="btn-group offset-right">
            <button type="button" v-if="showFolders && allowMove" class="btn btn-default" data-toggle="modal" :data-target="'#move_files_modal_'+this._uid">
                <i class="logoinc-move"></i>
                {{ __('logoinc::generic.move') }}
            </button>
            <button type="button" v-if="allowDelete" :disabled="selected_files.length == 0" class="btn btn-default" data-toggle="modal" :data-target="'#confirm_delete_modal_'+this._uid">
                <i class="logoinc-trash"></i>
                {{ __('logoinc::generic.delete') }}
            </button>
            <button v-if="allowCrop" :disabled="selected_files.length != 1 || !fileIs(selected_file, 'image')" type="button" class="btn btn-default" data-toggle="modal" :data-target="'#crop_modal_'+this._uid">
                <i class="logoinc-crop"></i>
                {{ __('logoinc::media.crop') }}
            </button>
        </div>
    </div>
        ...
        <div class="breadcrumb-container">
            <ol class="breadcrumb filemanager">
                <li class="media_breadcrumb" v-on:click="setCurrentPath(-1)">
                    ...
                    <strong>{{ __('logoinc::media.library') }}</strong>
                </li>
        ...
        <div class="flex">
            <div id="left">
                ...
                <div id="file_loader" v-if="is_loading">
                    ...
                    @endif
                    <p>{{ __('logoinc::media.loading') }}</p>
                </div>

                <div id="no_files" v-if="files.length == 0">
                    <h3><i class="logoinc-meh"></i> {{ __('logoinc::media.no_files_in_folder') }}</h3>
                </div>
            </div>
            <div id="right">
                <div class="right_details">
                    <div v-if="selected_files.length > 1" class="right_none_selected">
                        <i class="logoinc-list"></i>
                        <p>@{{ selected_files.length }} {{ __('logoinc::media.files_selected') }}</p>
                    </div>
                        ...
                        <div class="detail_img">
                            ...
                            <div v-else-if="fileIs(selected_file, 'video')">
                                <video width="100%" height="auto" ref="videoplayer" controls>
                                    ...
                                    {{ __('logoinc::media.browser_video_support') }}
                                </video>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'audio')">
                                <i class="logoinc-music"></i>
                                <audio controls style="width:100%; margin-top:5px;" ref="audioplayer">
                                    ...
                                    {{ __('logoinc::media.browser_audio_support') }}
                                </audio>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'zip')">
                                <i class="logoinc-archive"></i>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'folder')">
                                <i class="logoinc-folder"></i>
                            </div>
                            <div v-else>
                                <i class="logoinc-file-text"></i>
                            </div>
                        </div>
                        <div class="detail_info">
                            <span>
                                <h4>{{ __('logoinc::media.title') }}:</h4>
                            ...
                            <span>
                                <h4>{{ __('logoinc::media.type') }}:</h4>
                                <p>@{{ selected_file.type }}</p>
                            </span>

                            <template v-if="!fileIs(selected_file, 'folder')">
                                <span>
                                    <h4>{{ __('logoinc::media.size') }}:</h4>
                                    <p><span class="selected_file_size">@{{ bytesToSize(selected_file.size) }}</span></p>
                                </span>
                                <span>
                                    <h4>{{ __('logoinc::media.public_url') }}:</h4>
                                    <p><a :href="selected_file.path" target="_blank">{{ __('logoinc::generic.click_here') }}</a></p>
                                </span>
                                <span>
                                    <h4>{{ __('logoinc::media.last_modified') }}:</h4>
                    ...
                    <div v-else class="right_none_selected">
                        <i class="logoinc-cursor"></i>
                        <p>{{ __('logoinc::media.nothing_selected') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade modal-info" :id="'create_dir_modal_'+this._uid">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><i class="logoinc-folder"></i> {{ __('logoinc::media.add_new_folder') }}</h4>
                </div>

                <div class="modal-body">
                    <input name="new_folder_name" placeholder="{{ __('logoinc::media.new_folder_name') }}" class="form-control" value="" v-model="modals.new_folder.name" />
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('logoinc::generic.cancel') }}</button>
                    <button type="button" class="btn btn-info" v-on:click="createFolder">{{ __('logoinc::media.create_new_folder') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade modal-danger" :id="'confirm_delete_modal_'+this._uid" v-if="allowDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><i class="logoinc-warning"></i> {{ __('logoinc::generic.are_you_sure') }}</h4>
                </div>

                <div class="modal-body">
                    <h4>{{ __('logoinc::media.delete_question') }}</h4>
                    <ul>
                        <li v-for="file in selected_files">@{{ file.name }}</li>
                    </ul>
                    <h5 class="folder_warning">
                        <i class="logoinc-warning"></i> {{ __('logoinc::media.delete_folder_question') }}
                    </h5>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('logoinc::generic.cancel') }}</button>
                    <button type="button" class="btn btn-danger" v-on:click="deleteFiles">{{ __('logoinc::generic.delete_confirm') }}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="logoinc-move"></i> {{ __('logoinc::media.move_file_folder') }}</h4>
                </div>

                <div class="modal-body">
                    <h4>{{ __('logoinc::media.destination_folder') }}</h4>
                    <select class="form-control" v-model="modals.move_files.destination">
                        <option value="" disabled>{{ __('logoinc::media.destination_folder') }}</option>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('logoinc::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="moveFiles">{{ __('logoinc::generic.move') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade modal-warning" :id="'crop_modal_'+this._uid" v-if="allowCrop">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title">{{ __('logoinc::media.crop_image') }}</h4>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="new-image-info">
                        {{ __('logoinc::media.width') }} <span :id="'new-image-width_'+this._uid"></span>, {{ __('logoinc::media.height') }}<span :id="'new-image-height_'+this._uid"></span>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('logoinc::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(false)">{{ __('logoinc::media.crop') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(true)">{{ __('logoinc::media.crop_and_create') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    Vue.component('media-manager', {
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('logoinc.media.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    vm.files = [];
                ...
                if (file.type != 'folder') {
                    if (!this.allowMultiSelect) {
                        ...
                        if (content.length >= this.maxSelectedFiles && this.maxSelectedFiles > 0) {
                            var msg_sing = "{{ trans_choice('logoinc::media.max_files_select', 1) }}";
                            var msg_plur = "{{ trans_choice('logoinc::media.max_files_select', 2) }}";
            ...
            renameFile: function(object) {
                ...
                $.post('{{ route('logoinc.media.rename') }}', {
					...
					if (data.success == true) {
						toastr.success('{{ __('logoinc::media.success_renamed') }}', "{{ __('logoinc::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('logoinc::generic.whoopsie') }}");
					}
				});
            },
            createFolder: function(e) {
                ...
                $.post('{{ route('logoinc.media.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
						toastr.success('{{ __('logoinc::generic.successfully_created') }} ' + name, "{{ __('logoinc::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('logoinc::generic.whoopsie') }}");
					}
            ...
            deleteFiles: function() {
                ...
                var vm = this;
                $.post('{{ route('logoinc.media.delete') }}', {
                    path: vm.current_folder,
                    files: vm.selected_files,
                    _token: '{{ csrf_token() }}'
                }, function(data){
					if(data.success == true){
						toastr.success('', "{{ __('logoinc::generic.sweet_success') }}");
						vm.getFiles();
            ...
            moveFiles: function(e) {
				...
				$.post('{{ route('logoinc.media.move') }}', {
					...
					if(data.success == true){
						toastr.success('{{ __('logoinc::media.success_moved') }}', "{{ __('logoinc::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('logoinc::generic.whoopsie') }}");
					}
            ...
            crop: function(mode) {
                ...
                if (!mode) {
                    if (!window.confirm('{{ __('logoinc::media.crop_override_confirm') }}')) {
						return;
					}
                }
				...
				$.post('{{ route('logoinc.media.crop') }}', postData, function(data) {
					if (data.success) {
        ...
        mounted: function() {
            ...
            if (this.allowUpload && !dropzone.hasClass('dz-clickable')) {
                dropzone.dropzone({
                    timeout: 180000,
                    url: '{{ route('logoinc.media.upload') }}',
                    ...
                    success: function(e, res) {
                        if (res.success) {
                            toastr.success(res.message, "{{ __('logoinc::generic.sweet_success') }}");
                        } else {
                            toastr.error(res.message, "{{ __('logoinc::generic.whoopsie') }}");
                        }
                    },
                    error: function(e, res, xhr) {
                        toastr.error(res, "{{ __('logoinc::generic.whoopsie') }}");
                    },
                    ...
                    if (vm.hidden_element) {
                        if (vm.maxSelectedFiles > 1) {
                            ...
                            if (content.length < vm.minSelectedFiles) {
                                e.preventDefault();
                                var msg_sing = "{{ trans_choice('logoinc::media.min_files_select', 1) }}";
                                var msg_plur = "{{ trans_choice('logoinc::media.min_files_select', 2) }}";
                            ...
                            if (vm.minSelectedFiles > 0 && vm.hidden_element.value == '') {
                                e.preventDefault();
                                toastr.error("{{ trans_choice('logoinc::media.min_files_select', 1) }}");
                            }
                        }
                    }
                });
