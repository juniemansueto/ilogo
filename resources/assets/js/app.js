...
require('./logoinc_tinymce');
require('./logoinc_ace_editor');
...
$(document).ready(function () {
    ...
    $('#logoinc-loader').fadeOut();

    $(".hamburger, .navbar-expand-toggle").on('click', function () {
        appContainer.toggleClass("expanded");
        $(this).toggleClass('is-active');
        if ($(this).hasClass('is-active')) {
            window.localStorage.setItem('logoinc.stickySidebar', true);
        } else {
            window.localStorage.setItem('logoinc.stickySidebar', false);
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-collapse"]', function (e) {
        ...
        if (!$this.hasClass('panel-collapsed')) {
            ...
            $this.removeClass('logoinc-angle-up').addClass('logoinc-angle-down');
        } else {
            $this.parents('.panel').find('.panel-body').slideDown();
            $this.removeClass('panel-collapsed');
            $this.removeClass('logoinc-angle-down').addClass('logoinc-angle-up');
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-fullscreen"]', function (e) {
        e.preventDefault();
        var $this = $(this);
        if (!$this.hasClass('logoinc-resize-full')) {
            $this.removeClass('logoinc-resize-small').addClass('logoinc-resize-full');
        } else {
            $this.removeClass('logoinc-resize-full').addClass('logoinc-resize-small');
        }
